...
class Printer {
  ...
  print() {
    ...
    const imports = group([
      'import { request } from "umi";', hardline,
      'import {',
      indent([
        hardline,
  ...
  request(path, method, operation) {
    ...
    return [
      ...
      indent([
        ...
        [
          funcBody,
          `return request`, this.responseType(operation.responses), `(\`${pathName}\`, {`,
          indent([
            hardline,
...
function print(t, options) {
  ...
  const printOptions = {printWidth: 80, tabWidth: 2}
  const code = printDocToString(doc, printOptions).formatted
  return [
    {
      fileName: 'request.ts',
      code: code,
